...
export const getAllPenndingLoansController = async (req: Request, res: Response) => {
    try {
        const loans= await prisma.loan.findMany({where:{returnedAt:null}}).then(loans => loans.length);
        res.status(200).json({loans});
    } catch (error) {
        res.status(500).json({message:"internal server error"})
    }
